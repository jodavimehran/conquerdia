...
public class Player implements Serializable {
	...
	public ArrayList<String> attack(String fromCountryName, String toCountryName, int numdice, boolean isAllOut)
		...
		if (!isAllOut) {
			...
			if (numdice >= fromCountry.getNumberOfArmies()) {
				throw new ValidationException(String.format(
						"Number of dice rolled (%d) should be less than the number of armies (%d) in \"%s\")",
						numdice, fromCountry.getNumberOfArmies(), fromCountry.getName()));
			}
	...
	private void validateCountries(String fromCountryName, String toCountryName)
		...
		if (this.owns(toCountryName)) {
			throw new ValidationException(String.format("You cannot attack your own country %s.", toCountryName));
		}
		...
		// Attacker need to keep atleast one army
		if (fromCountry.getNumberOfArmies() < 2) {
			throw new ValidationException(
					String.format("You must have atleast 2 armies in %s to attack %s.", fromCountry, toCountry));
		}
	}
	...
	public ArrayList<String> defend(int numDice) throws ValidationException {
		...
		if (numDice > 2) {
			error = "Defender cannot roll more than 2 dices and not more than the number of armies contained in defending country";
		} else if (!isInBattle()) {
